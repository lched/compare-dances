...
def DisplayAnimationLayer(pAnimLayer, pNode, isSwitcher=False):
    ...
    DisplayChannels(
        pNode, pAnimLayer, DisplayCurveKeys, DisplayListCurveKeys, isSwitcher
    )
    print
...
def DisplayChannels(pNode, pAnimLayer, DisplayCurve, DisplayListCurve, isSwitcher):
    ...
    if lNodeAttribute:
        ...
        if (
            ...
            or lNodeAttribute.GetAttributeType() == FbxNodeAttribute.eNurbs
            or lNodeAttribute.GetAttributeType() == FbxNodeAttribute.ePatch
        ):
    ...
    while lProperty.IsValid():
        if lProperty.GetFlag(FbxPropertyFlags.eUserDefined):
            ...
            if (
                lDataType.GetType() == eFbxBool
                or lDataType.GetType() == eFbxDouble
                or lDataType.GetType() == eFbxFloat
                or lDataType.GetType() == eFbxInt
            ):
                lMessage = "        Property "
                ...
                lDataType.GetType() == eFbxDouble3
                or lDataType.GetType() == eFbxDouble4
