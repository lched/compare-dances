...
def load(filename, start=None, end=None, order=None, world=True):
    ...
    for line in f:
        ...
        if chanmatch:
            ...
            if (
                not order
            ):  # do NOT ask 'if order is NONE' because order may be an empty srint ('')
                ...
                order = print_order[
                    ::-1
                ]  # in a bvh file, first rotation axis is printed last
            continue

        jmatch = re.match(
            "\s*JOINT\s+(\S+)", line
        )  #  match <white-spaces>Joint<white-spaces><non-white-spaces>, .e.g: Joint mixamorig:LeftArm
        if jmatch:
    ...
    quat_rotations = Quaternions.from_euler(
...
def save(
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            ...
            % (
                t,
                channelmap_inv[print_order[0]],
                channelmap_inv[print_order[1]],
                channelmap_inv[print_order[2]],
            )
        )
                    ...
                    if positions or j == 0:
                        f.write(
                            ...
                            % (
                                ...
                                poss[i, j, 1],
                                poss[i, j, 2],
                                rots[i, j, ordermap[print_order[0]]],
                                rots[i, j, ordermap[print_order[1]]],
                                rots[i, j, ordermap[print_order[2]]],
                            )
                        )
